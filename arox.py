...
class SocksServer(SocksBase):
    def __init__(self, *args, **kwargs):
        super(SocksServer, self).__init__(*args, **kwargs)
        signal.signal(signal.SIGQUIT, self.stats_handler)

    def stats_handler(self, *args):
        self.logger.warning('')
        self.logger.warning('~~~ Stats for nerds : {} open channels, tunnel peer is {}:{} ~~~'.format(len(self.tunnel.channels), *self.tunnel.transport_socket.getpeername()[:2]))
        for c in self.tunnel.channels:
            self.logger.warning('  {}'.format(c))
        self.logger.warning('~~~ End of Stats ~~~')

    def run(self):
        while self.tunnel is None:
            s = socket.socket()
            s = ssl.wrap_socket(s, server_side=True, certfile=self.cert, keyfile=self.key)
            s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
            s.bind(self.transport_addr)
            s.listen(1)
            self.logger.warning('Waiting for Tunnel connections on {}:{}'.format(*self.transport_addr))
            c, a = s.accept()

            if True:  # TODO: Verify the client
                self.logger.warning('Accepted Tunnel connection from {}:{}'.format(*a[:2]))
                self.tunnel = Tunnel(c)
                break

        self.socks_socket = socket.socket()
        self.socks_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        self.socks_socket.bind(self.socks_addr)
        self.socks_socket.listen(100)
        self.logger.info('Listening for SOCKSv5 clients on {}:{}'.format(*self.socks_addr))

        self.monitor_sockets()
        self.logger.warning('SOCKS server is exiting')
...
def server_main(args):
    tunnel_addr = (args.tunnel_addr.split(':')[0], int(args.tunnel_addr.split(':')[1]))
    socks_addr = (args.socks_addr.split(':')[0], int(args.socks_addr.split(':')[1]))
    server = SocksServer(transport_addr=tunnel_addr, socks_addr=socks_addr, keepalive=args.keepalive, cert=args.cert, key=args.key)
    server.run()
    return
...
def main():
    ...
    server_parser.add_argument('-s', '--socks-addr', type=str, default='0.0.0.0:1080',
                               help='The address to bind for the SOCKS server (default is 0.0.0.0:1080)')
    server_parser.add_argument('-t', '--tunnel-addr', type=str, default='0.0.0.0:4433',
                               help='The address to bind for the tunnel (default is 0.0.0.0:4433)')
    server_parser.add_argument('--cert', required=True, help='File containing server TLS certificate')
    server_parser.add_argument('--key', required=True, help='File containing server TLS private key')
    server_parser.add_argument('--keepalive', type=float, default=60.0, help='Tunnel keepalive messages')
    ...
    relay_parser.add_argument('--tunnel-addr', required=True, help='The server to connect to (Format is host:port).')
